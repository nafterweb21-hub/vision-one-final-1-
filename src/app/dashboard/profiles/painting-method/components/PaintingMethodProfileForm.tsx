...
interface PaintingMethodProfileFormProps {
  editingProfile?: PaintingMethodProfile | null;
}

export default function PaintingMethodProfileForm({ editingProfile }: PaintingMethodProfileFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingProfile 
        ? `/api/profiles/painting-method/${editingProfile.id}` 
        : "/api/profiles/painting-method";
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error || "Failed to save Painting Method.");
      }

      // Navigate back with success toast query parameter
      const msg = editingProfile ? "updated" : "created";
      router.push(`/dashboard/profiles/painting-method?toast=${msg}`);
      router.refresh();
    } catch (err: any) {
      ...
      <form onSubmit={handleSubmit} className="flex flex-col py-8 px-8 space-y-6">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-1.5">
            ...
            />
            <p className="text-[10px] text-blue-400">
              Optional description for the Painting Method.
            </p>
          </div>
        ...
        <div className="border-t border-blue-100 pt-6 flex items-center justify-between ">
          <button
            type="button"
            onClick={() => router.push("/dashboard/profiles/painting-method")}
            disabled={saving}
