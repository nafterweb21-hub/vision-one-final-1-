...
export default function materialTypesPage() {
  ...
  const [types, setCategories] = useState<PaintingMethodProfile[]>([]);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setErrorMessage(err.message || "An unexpected error occurred while loading painting methods.");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (toastType === "created") {
      triggerToast("New Painting Method created successfully.");
      router.replace("/dashboard/profiles/painting-method");
    } else if (toastType === "updated") {
      triggerToast("Painting Method updated successfully.");
      router.replace("/dashboard/profiles/painting-method");
    }
  }, [toastType, router]);
  ...
  const handleCreateOpen = () => {
    router.push("/dashboard/profiles/painting-method/create");
  };

  // Navigate to Edit page
  const handleEditOpen = (method: PaintingMethodProfile) => {
    router.push(`/dashboard/profiles/painting-method/edit/${method.id}`);
  };
  ...
  const handleToggleStatus = async (id: string, currentStatus: string) => {
    ...
    try {
      const res = await fetch(`/api/profiles/painting-method/${id}`, {
        method: "PATCH",
      ...
      triggerToast(`Painting Method status successfully set to ${nextStatus}.`);
    } catch (err: any) {
  ...
  const handleVoidMethod = async (id: string) => {
    if (!await customConfirm("Are you sure you want to VOID this Painting Method? Voided records cannot be restored and running audit logs are permanently maintained.")) {
      return;
    }
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed to void painting method.");

      setCategories((prev) =>
        prev.map((c) => (c.id === id ? { ...c, status: "Void" } : c))
      );
      triggerToast("Painting Method voided successfully.");
    } catch (err: any) {
      hotToast.error(err.message || "Failed to void painting method.");
    }
  };

  // Filter types by status
  const activeCategories = types.filter((c) => c.status !== "Void");
  const voidedCategories = types.filter((c) => c.status === "Void");
    ...
    <div className="space-y-6 animate-fade-in relative">
      ...
      {loading ? (
        <div className="flex h-64 flex-col items-center justify-center rounded-2xl border border-blue-200 bg-white shadow-sm">
          ...
          <span className="mt-4 text-xs font-medium text-blue-500">Retrieving painting methods...</span>
        </div>
      ) : errorMessage && types.length === 0 ? (
          ...
          <div className="overflow-hidden rounded-2xl border border-blue-200 bg-white shadow-sm ">
            ...
            {activeCategories.length === 0 ? (
              <div className="p-8 text-center text-xs text-blue-500 ">
                No active painting methods configured. Click "Create Painting Method" to add one.
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-blue-200 text-left">
                  <thead className="bg-blue-50 text-xs font-semibold text-blue-500 uppercase tracking-wider">
                    <tr>
                      <th className="px-6 py-3.5">Painting Method</th>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-blue-200 text-left">
                  <thead className="bg-blue-50 text-xs font-semibold text-blue-500">
                    <tr>
                      <th className="px-6 py-2">Painting Method</th>
                  ...
                  <tbody className="divide-y divide-blue-100 bg-white text-xs text-blue-600">
                    {voidedCategories.map((c) => (
                      <tr key={c.id}>
                        <td className="px-6 py-2.5 font-semibold">{c.method}</td>
