...
export default function materialJointsPage() {
  const router = useRouter();
  const searchParams = useSearchParams();

  const [profiles, setCategories] = useState<JointProfile[]>([]);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setErrorMessage(err.message || "An unexpected error occurred while loading joint profiles.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const toastJoint = searchParams.get("toast");
  useEffect(() => {
    if (toastJoint === "created") {
      triggerToast("New Joint Profile created successfully.");
      router.replace("/dashboard/profiles/joint-profiles");
    } else if (toastJoint === "updated") {
      triggerToast("Joint Profile updated successfully.");
      router.replace("/dashboard/profiles/joint-profiles");
    }
  }, [toastJoint, router]);
  ...
  const handleCreateOpen = () => {
    router.push("/dashboard/profiles/joint-profiles/create");
  };

  // Navigate to Edit page
  const handleEditOpen = (joint: JointProfile) => {
    router.push(`/dashboard/profiles/joint-profiles/edit/${joint.id}`);
  };
  ...
  const handleToggleStatus = async (id: string, currentStatus: string) => {
    ...
    try {
      const res = await fetch(`/api/profiles/joint-profiles/${id}`, {
        method: "PATCH",
      ...
      triggerToast(`joint status successfully set to ${nextStatus}.`);
    } catch (err: any) {
      hotToast.error(err.message || "Could not toggle status.");
    }
  };

  // Void a joint (No delete - Void only)
  const handleVoidjoint = async (id: string) => {
    if (!await customConfirm("Are you sure you want to VOID this Joint Profile? Voided records cannot be restored and running audit logs are permanently maintained.")) {
      return;
    }
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed to void joint.");

      setCategories((prev) =>
        prev.map((c) => (c.id === id ? { ...c, status: "Void" } : c))
      );
      triggerToast("Joint Profile voided successfully.");
    } catch (err: any) {
      hotToast.error(err.message || "Failed to void joint.");
    }
  };

  // Filter profiles by status
  const activeCategories = profiles.filter((c) => c.status !== "Void");
  const voidedCategories = profiles.filter((c) => c.status === "Void");
    ...
    <div className="space-y-6 animate-fade-in relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold text-indigo-600 uppercase tracking-wider">
            ...
            <span>/</span>
            <span>Joint Profile Profile</span>
          </div>
          <h2 className="mt-1 text-2xl font-bold tracking-tight text-blue-900 ">
            Joint Profile Configuration
          </h2>
          <p className="mt-1 text-xs text-blue-500 ">
            Define and manage group profiles to classify material catalog records.
          </p>
        </div>
        <button
      ...
      {loading ? (
        <div className="flex h-64 flex-col items-center justify-center rounded-2xl border border-blue-200 bg-white shadow-sm">
          ...
          <span className="mt-4 text-xs font-medium text-blue-500">Retrieving joint profiles...</span>
        </div>
        ...
        <div className="space-y-8">
          ...
          <div className="overflow-hidden rounded-2xl border border-blue-200 bg-white shadow-sm ">
            <div className="border-b border-blue-100 bg-blue-50/50 px-6 py-4.5 ">
              ...
              <p className="text-[11px] text-blue-500 ">Manage profiles configured across the ERP</p>
            </div>
            
            {activeCategories.length === 0 ? (
              <div className="p-8 text-center text-xs text-blue-500 ">
                No active joint profiles configured. Click "Create Joint Profile" to add one.
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-blue-200 text-left">
                  <thead className="bg-blue-50 text-xs font-semibold text-blue-500 uppercase tracking-wider">
                    <tr>
                      <th className="px-6 py-3.5">joint Name</th>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-blue-200 text-left">
                  <thead className="bg-blue-50 text-xs font-semibold text-blue-500">
                    <tr>
                      <th className="px-6 py-2">joint Name</th>
                  ...
                  <tbody className="divide-y divide-blue-100 bg-white text-xs text-blue-600">
                    {voidedCategories.map((c) => (
                      <tr key={c.id}>
                        <td className="px-6 py-2.5 font-semibold">{c.joint}</td>
