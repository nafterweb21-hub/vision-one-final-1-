...
interface WeldingTypeFormProps {
  editingProfile?: WeldingType | null;
}

export default function WeldingTypeForm({ editingProfile }: WeldingTypeFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingProfile 
        ? `/api/profiles/welding-types/${editingProfile.id}` 
        : "/api/profiles/welding-types";
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error || "Failed to save Welding Type.");
      }

      // Navigate back with success toast query parameter
      const msg = editingProfile ? "updated" : "created";
      router.push(`/dashboard/profiles/welding-types?toast=${msg}`);
      router.refresh();
    } catch (err: any) {
      ...
      <form onSubmit={handleSubmit} className="flex flex-col py-8 px-8 space-y-6">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-1.5">
            ...
            />
            <p className="text-[10px] text-blue-400">
              Optional description for the Welding Type.
            </p>
          </div>
        ...
        <div className="border-t border-blue-100 pt-6 flex items-center justify-between ">
          <button
            type="button"
            onClick={() => router.push("/dashboard/profiles/welding-types")}
            disabled={saving}
