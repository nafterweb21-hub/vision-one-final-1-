...
export default function materialTypesPage() {
  ...
  const [types, setCategories] = useState<WeldingType[]>([]);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setErrorMessage(err.message || "An unexpected error occurred while loading welding types.");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (toastType === "created") {
      triggerToast("New Welding Type created successfully.");
      router.replace("/dashboard/profiles/welding-types");
    } else if (toastType === "updated") {
      triggerToast("Welding Type updated successfully.");
      router.replace("/dashboard/profiles/welding-types");
    }
  }, [toastType, router]);
  ...
  const handleCreateOpen = () => {
    router.push("/dashboard/profiles/welding-types/create");
  };

  // Navigate to Edit page
  const handleEditOpen = (type: WeldingType) => {
    router.push(`/dashboard/profiles/welding-types/edit/${type.id}`);
  };
  ...
  const handleToggleStatus = async (id: string, currentStatus: string) => {
    ...
    try {
      const res = await fetch(`/api/profiles/welding-types/${id}`, {
        method: "PATCH",
  ...
  const handleVoidtype = async (id: string) => {
    if (!await customConfirm("Are you sure you want to VOID this Welding Type? Voided records cannot be restored and running audit logs are permanently maintained.")) {
      return;
    }
    ...
    try {
      ...
      triggerToast("Welding Type voided successfully.");
    } catch (err: any) {
      hotToast.error(err.message || "Failed to void type.");
    }
  };
    ...
    <div className="space-y-6 animate-fade-in relative">
      ...
      {loading ? (
        <div className="flex h-64 flex-col items-center justify-center rounded-2xl border border-blue-200 bg-white shadow-sm">
          ...
          <span className="mt-4 text-xs font-medium text-blue-500">Retrieving welding types...</span>
        </div>
          ...
          <div className="overflow-hidden rounded-2xl border border-blue-200 bg-white shadow-sm ">
            ...
            {activeCategories.length === 0 ? (
              <div className="p-8 text-center text-xs text-blue-500 ">
                No active welding types configured. Click "Create Welding Type" to add one.
              </div>
            ) : (
