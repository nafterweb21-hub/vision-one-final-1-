...
import {
  ...
  toggleTermsConditionStatus,
  deleteTermsConditionProfile,
} from "./actions";

interface TermsAndConditionProfile {
  id: string;
  name: string;
  content: string;
  remark: string | null;
...
export default function TermsAndConditionProfilePage() {
  const [items, setItems] = useState<TermsAndConditionProfile[]>([]);
  ...
  const loadData = async () => {
    ...
    const res = await getTermsConditionsList();

    if (res.success && res.data) {
      setItems(res.data as unknown as TermsAndConditionProfile[]);
    } else {
      setErrorMsg(res.error || "Failed to load terms and conditions.");
    }
  ...
  const handleToggleStatus = async (id: string) => {
    startTransition(async () => {
      const res = await toggleTermsConditionStatus(id);
      if (res.success) {
        loadData();
      } else {
  ...
  const handleDelete = async (id: string) => {
    if (await customConfirm("Are you sure you want to delete this profile?")) {
      startTransition(async () => {
        const res = await deleteTermsConditionProfile(id);
        if (res.success) {
          loadData();
        } else {
          hotToast.error(res.error || "Failed to delete profile.");
        }
      });
    }
  };
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-blue-500 mt-1">
            Maintain the master list of terms and conditions presets.
          </p>
        </div>

        <Link
          href="/dashboard/master-profile/terms-condition/new"
      ...
      <div className="bg-white rounded-2xl border border-blue-200 overflow-hidden shadow-sm flex-1 flex flex-col">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center flex-1 gap-2 p-6 text-center">
            ...
            <p className="font-semibold text-blue-900">Error loading profiles</p>
          ...
          <div className="flex flex-col items-center justify-center flex-1 p-6 text-center">
            ...
            <p className="mt-3 font-semibold text-blue-900">No profiles found</p>
            <p className="text-sm text-blue-500 mt-1">
              {searchQuery ? "No matches for your criteria." : "Create your first profile to begin."}
            </p>
          </div>
        ) : (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          href={`/dashboard/master-profile/terms-condition/${item.id}`}
