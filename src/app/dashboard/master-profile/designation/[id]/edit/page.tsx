...
import { getDesignationProfileItems, updateDesignationProfileItem } from "../../actions";

export default function EditDesignationProfilePage() {
    ...
    async function loadItem() {
      ...
      if (res.success && res.data) {
        ...
        if (item) {
          ...
          setErrorMsg("Designation profile not found.");
        }
      } else {
        setErrorMsg(res.error || "Failed to load designation profile data.");
      }
      setIsLoading(false);
    }
    loadItem();
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    formData.set("designation", designation); // pass the disabled field

    startTransition(async () => {
      const res = await updateDesignationProfileItem(id, formData);
      if (res.success) {
        router.push("/dashboard/master-profile/designation");
      } else {
  ...
  return (
    <div className="flex-1 flex flex-col p-6 space-y-6 max-w-3xl mx-auto w-full">
      <div className="flex items-center gap-4">
        <Link
          href="/dashboard/master-profile/designation"
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-blue-900 ">
            Edit Designation Profile
          </h1>
          <p className="text-sm text-blue-500 mt-1">
            Update remarks for the selected designation profile.
          </p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            <div className="grid gap-2">
              <label htmlFor="designation" className="text-sm font-bold text-blue-900">
                Designation
              </label>
              <input
                id="designation"
              ...
              <p className="text-xs text-blue-500">
                Designation name cannot be edited after creation.
              </p>
            </div>
