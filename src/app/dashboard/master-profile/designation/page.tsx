...
import {
  getDesignationProfileItems,
  toggleDesignationProfileStatus,
  deleteDesignationProfileItem,
} from "./actions";

interface DesignationProfile {
  id: string;
  designation: string;
  remark: string | null;
  status: string;
...
export default function DesignationProfilePage() {
  const [items, setItems] = useState<DesignationProfile[]>([]);
  ...
  const loadData = async () => {
    ...
    if (res.success && res.data) {
      setItems(res.data as DesignationProfile[]);
    } else {
      setErrorMsg(res.error || "Failed to load designation profiles.");
    }
  ...
  const handleToggleStatus = async (id: string) => {
    startTransition(async () => {
      const res = await toggleDesignationProfileStatus(id);
      if (res.success) {
        loadData();
      } else {
  ...
  const handleDelete = async (id: string) => {
    if (await customConfirm("Are you sure you want to delete this designation profile?")) {
      startTransition(async () => {
        const res = await deleteDesignationProfileItem(id);
        if (res.success) {
          loadData();
        } else {
          hotToast.error(res.error || "Failed to delete designation profile.");
        }
      });
    }
  };
    ...
    <div className="flex-1 flex flex-col p-6 space-y-6 max-w-7xl mx-auto w-full bg-blue-50 text-blue-900 ">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-blue-500 mt-1">
            Maintain the master list of employee designations used in the system.
          </p>
        </div>

        <Link
          href="/dashboard/master-profile/designation/new"
      ...
      <div className="rounded-xl border border-blue-200 bg-white p-4 flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            type="text"
            placeholder="Search by designation or remark..."
            value={searchQuery}
      ...
      <div className="bg-white rounded-2xl border border-blue-200 overflow-hidden shadow-sm flex-1 flex flex-col">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center flex-1 gap-2 p-6 text-center">
            ...
            <p className="font-semibold text-blue-900 ">Error loading designation profiles</p>
          ...
          <div className="flex flex-col items-center justify-center flex-1 p-6 text-center">
            ...
            <p className="mt-3 font-semibold text-blue-900 ">No designation profiles found</p>
            <p className="text-sm text-blue-500 mt-1">
              {searchQuery || statusFilter !== "All" ? "No matches for your criteria." : "Create your first Designation Profile to begin."}
            </p>
          </div>
        ) : (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          href={`/dashboard/master-profile/designation/${item.id}/edit`}
