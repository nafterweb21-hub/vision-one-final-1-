...
import { getInvoices, getInvoice, voidInvoice, submitInvoice, createInvoice, reviseInvoice, generateEInvoice, cancelEInvoice } from "./debit-note.actions";
...
export default function DebitNoteListPage() {
  ...
  const onEdit = () => {
    ...
    router.push(`/dashboard/sales/debit-note/form?id=${selected.id}`);
  };
  ...
  const onRevise = async () => {
    ...
    if (res.success && res.data) {
      router.push(`/dashboard/sales/debit-note/form?id=${res.data.id}`);
    } else {
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 pb-6 border-b border-blue-200">
        ...
        <Link
          href="/dashboard/sales/debit-note/form"
      ...
      {loading ? (
        ...
        <div className="bg-white border border-blue-200 rounded-xl p-12 text-center shadow-sm">
          ...
          <p className="text-xs text-blue-400 mt-1">
            Click &quot;New Debit Note&quot; to create your first invoice.
          </p>
        </div>
      ) : (
                ...
                {filteredInvoices.map((inv) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3 font-bold text-blue-900">
                      <Link
                        href={`/dashboard/sales/debit-note/form?id=${inv.id}`}
